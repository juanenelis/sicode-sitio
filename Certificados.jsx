...
//
// El acceso para subir certificados se valida contra las cuentas reales
// del Aula Virtual (director, profesor, admin2, secretaria) guardadas en
// Firestore, además de aceptar la clave general de administrador del
// sitio como respaldo.
// ============================================================
...
const inputStyle = { width: "100%", background: "#FFFFFF", border: `1px solid ${C.line}`, borderRadius: 10, padding: "12px 14px", color: C.text, fontSize: 14.5, fontFamily: FB, outline: "none" };

// Convierte la URL de Cloudinary para que el navegador SIEMPRE descargue el
// archivo (en vez de intentar abrirlo dentro del navegador, que en algunos
// celulares/navegadores falla con documentos). Así, quien lo reciba lo baja
// como un archivo normal y lo abre después con su propio lector de PDF.
...
async function verificarCredencialStaff(usuario, password) {
  const u = (usuario || "").trim().toLowerCase();
  const p = password || "";
  if (!u || !p) return false;
  try {
    const snap = await getDocs(collection(db, "aula_usuarios"));
    const rolesConAcceso = ["director", "maestro", "admin2", "secretaria"];
    for (const d of snap.docs) {
      const cuenta = d.data();
      const coincideUsuario = (cuenta.correoLogin || "").toLowerCase() === u || (cuenta.cedula || "") === (usuario || "").trim();
      if (coincideUsuario && cuenta.password === p && rolesConAcceso.includes(cuenta.rol) && !cuenta.suspendido) {
        return true;
      }
    }
    return false;
...
export default function Certificados({ checkAdminCreds, isAdmin }) {
  const [modoSubir, setModoSubir] = useState(false);
  const [staffAutenticado, setStaffAutenticado] = useState(false);
  const autenticado = isAdmin || staffAutenticado;
  ...
  return (
    <div style={{ background: C.navy2, padding: "8vh 8vw", minHeight: "60vh" }}>
      ...
      {modoSubir ? (
        <PanelSubida isAdmin={autenticado} setIsAdmin={setStaffAutenticado} checkAdminCreds={checkAdminCreds} onCerrar={() => setModoSubir(false)} />
      ) : (
      ...
      {resultados !== null && (
        resultados.length === 0 ? (
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            {resultados.map((c) => (
              <a key={c.key} href={urlDescarga(c.url)} download style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12, background: C.panel, border: `1px solid ${C.line}`, borderRadius: 12, padding: "14px 16px", textDecoration: "none" }}>
...
function PanelSubida({ isAdmin, setIsAdmin, checkAdminCreds, onCerrar }) {
  const [user, setUser] = useState(""); const [pwd, setPwd] = useState("");
  const [pwdError, setPwdError] = useState(false);
  const [verificando, setVerificando] = useState(false);

  const checkPwd = async (e) => {
    e.preventDefault();
    setPwdError(false); setVerificando(true);
    try {
      const esStaffReal = await verificarCredencialStaff(user, pwd);
      if (esStaffReal || checkAdminCreds(user, pwd)) {
        setIsAdmin(true);
      } else {
        setPwdError(true);
      }
    ...
    return (
      <div style={{ maxWidth: 380 }}>
        <form onSubmit={checkPwd} style={{ background: C.panel, border: `1px solid ${C.line}`, borderRadius: 14, padding: 22, display: "flex", flexDirection: "column", gap: 12 }}>
          ...
          <p style={{ color: C.textDim, fontSize: 11.5, margin: 0 }}>Usa tu usuario y contraseña del Aula Virtual.</p>
          <input style={inputStyle} placeholder="Usuario (correo institucional o cédula)" value={user} onChange={(e) => setUser(e.target.value)} />
          ...
          <div style={{ display: "flex", gap: 8 }}>
            <button type="submit" disabled={verificando} style={{ background: C.gold, color: "#141A22", border: "none", borderRadius: 10, padding: "10px 16px", fontWeight: 700, cursor: verificando ? "default" : "pointer", opacity: verificando ? 0.6 : 1, display: "flex", alignItems: "center", gap: 8 }}>
              {verificando ? <Loader2 size={14} className="animate-spin" /> : null} {verificando ? "Verificando…" : "Entrar"}
            </button>
