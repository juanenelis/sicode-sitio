...
// Rango de cada rol: un usuario solo puede suspender/eliminar/editar a otro
// usuario si su rango es ESTRICTAMENTE MAYOR al del usuario objetivo. Esto evita
// que, por ejemplo, un profesor pueda eliminar al director o a otro profesor.
const RANGO_ROL = {
  director: 4, admin2: 3, secretaria: 2, maestro: 1, estudiante: 0, visitante: 0,
};
...
function FichaUsuarioModal({ usuario, sesion, setUsuarios, onClose }) {
  const puedeAdministrar = !!setUsuarios && sesion && ["director", "maestro", "admin2", "secretaria"].includes(sesion.rol) && usuario.correoLogin !== sesion.correoLogin && (RANGO_ROL[sesion.rol] ?? 0) > (RANGO_ROL[usuario.rol] ?? 0);
    ...
    <Card maxWidth={420}>
      <div style={{ textAlign: "center", marginBottom: 16 }}>
        <label style={{ cursor: "pointer", display: "inline-block", position: "relative" }}>
          ...
          <input type="file" accept="image/*" style={{ display: "none" }} onChange={cambiarFoto} />
        </label>
...
function EstudianteDashboard({ sesion, setSesion, centro, usuarios, setUsuarios, cursos, setCursos, biblioteca, setBiblioteca, chats, setChats, grupos, setGrupos, alertas, onSalir }) {
  ...
  const cambiarFoto = (e) => {
    ...
    setUsuarios((us) => us.map((u) => u.correoLogin === sesion.correoLogin ? { ...u, fotoPerfil: url } : u));
    setSesion((s) => ({ ...s, fotoPerfil: url }));
  };
