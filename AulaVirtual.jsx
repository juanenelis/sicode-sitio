...
import { useFirestoreState, useFirestoreCollection, migrarDatosAntiguos } from "./aulaFirestore";
...
// Un estudiante puede estar inscrito en hasta 3 cursos al mismo tiempo.
const MAX_CURSOS_ESTUDIANTE = 3;
function cursosDe(usuario) { return usuario?.cursosIds || []; }
function estaEnCurso(usuario, cursoId) { return cursosDe(usuario).includes(cursoId); }
...
export default function AulaVirtual() {
  ...
  useEffect(() => { migrarDatosAntiguos().finally(() => setMigrado(true)); }, []);

  const [centro, setCentro] = useFirestoreState("centro", centroInicial);
  const [usuarios, setUsuarios] = useFirestoreCollection("aula_usuarios", (u) => u.correoLogin, usuariosInicial, migrado);
  const [cursos, setCursos] = useFirestoreCollection("aula_cursos", (c) => c.id, [], migrado);
  const [biblioteca, setBiblioteca] = useFirestoreCollection("aula_biblioteca", (b) => b.id, [], migrado);
  const [chats, setChats] = useFirestoreState("chats", {});
  const [grupos, setGrupos] = useFirestoreCollection("aula_grupos", (g) => g.id, [], migrado);
  const [alertas, setAlertas] = useFirestoreCollection("aula_alertas", (a) => a.id, [], migrado);
  ...
  return (
    <div style={{ minHeight: "70vh", background: C.navy, fontFamily: FB, borderRadius: 20, position: "relative", overflow: "hidden" }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", alignItems: "center", padding: "3vh 5vw 5vh", position: "relative", zIndex: 2 }}>
        {!migrado ? (
          <div style={{ color: C.textDim, fontSize: 13, padding: "10vh 0", display: "flex", alignItems: "center", gap: 10 }}>
            <span style={{ width: 16, height: 16, border: `2px solid ${C.line}`, borderTopColor: C.blueLight, borderRadius: "50%", display: "inline-block", animation: "spin 0.8s linear infinite" }} />
            Cargando el Aula Virtual…
          </div>
        ) : (
        <>
        {!sesion && view === "login" && (
...
function RegistroEstudianteIndependiente({ centro, setUsuarios, cursos, onRegistrado, goLogin }) {
  ...
  const submit = () => {
    ...
    const nuevoUsuario = {
      correoLogin, cedula, password, rol: "estudiante", centroId: centro.id, cursosIds: [curso.id],
...
function Chat({ sesion, centro, usuarios, setUsuarios, chats, setChats, grupos, setGrupos }) {
  ...
  const [leidoHasta, setLeidoHasta] = useState({}); // { claveConversacion: timestamp }
  const key = centro.id;

  const nombreDe = (correo) => correo === sesion.correoLogin ? sesion.nombre : (usuarios.find((u) => u.correoLogin === correo)?.nombre || correo);

  const todosMensajes = chats[key] || [];
  const mensajesDirectoCon = (correo) => todosMensajes.filter((m) => !m.grupoId && ((m.con === correo && m.de === sesion.correoLogin) || (m.de === correo && m.con === sesion.correoLogin)));
  const mensajesDeGrupo = (grupoId) => todosMensajes.filter((m) => m.grupoId === grupoId);
  const ultimoTs = (msgs) => msgs.length ? Math.max(...msgs.map((m) => m.ts || 0)) : 0;
  const noLeidos = (msgs, claveConv) => msgs.filter((m) => m.de !== sesion.correoLogin && (m.ts || 0) > (leidoHasta[claveConv] || 0)).length;

  const contactosFiltrados = contactos
    .filter((c) => coincide(c, busqueda))
    .map((c) => ({ contacto: c, msgs: mensajesDirectoCon(c.correoLogin) }))
    .sort((a, b) => ultimoTs(b.msgs) - ultimoTs(a.msgs));

  const gruposOrdenados = misGrupos
    .map((g) => ({ grupo: g, msgs: mensajesDeGrupo(g.id) }))
    .sort((a, b) => ultimoTs(b.msgs) - ultimoTs(a.msgs));
  ...
  // Al abrir una conversación, la marcamos como leída hasta ahora.
  useEffect(() => {
    if (!activo) return;
    const clave = activo.tipo === "grupo" ? `grupo-${activo.id}` : activo.id;
    setLeidoHasta((lh) => ({ ...lh, [clave]: Date.now() }));
  }, [activo]);

  const enviar = () => {
    if (!texto.trim() || !activo) return;
    const base = { de: sesion.correoLogin, deNombre: sesion.nombre, deRol: sesion.rol, texto, fecha: new Date().toLocaleTimeString(), ts: Date.now() };
      ...
      <div style={{ width: 210, borderRight: `1px solid ${C.line}`, overflowY: "auto", display: "flex", flexDirection: "column", gap: 4, paddingRight: 8 }}>
        ...
        {creandoGrupo && (
          <div style={{ background: C.navy2, border: `1px solid ${C.line}`, borderRadius: 10, padding: 10, marginBottom: 10, display: "flex", flexDirection: "column", gap: 8 }}>
            ...
            <div style={{ maxHeight: 130, overflowY: "auto", display: "flex", flexDirection: "column", gap: 5 }}>
              {contactos.filter((c) => coincide(c, busqueda)).map((c) => (
        ...
        {misGrupos.length > 0 && (
          <div style={{ marginBottom: 8 }}>
            ...
            {gruposOrdenados.map(({ grupo: g, msgs }) => {
              const clave = `grupo-${g.id}`;
              const activoAqui = activo?.tipo === "grupo" && activo.id === g.id;
              const sinLeer = activoAqui ? 0 : noLeidos(msgs, clave);
              return (
                <button key={g.id} onClick={() => setActivo({ tipo: "grupo", id: g.id })} style={{
                  textAlign: "left", width: "100%", background: activoAqui ? C.navy2 : "none", border: "none", borderRadius: 9,
                  padding: "8px 10px", cursor: "pointer", color: C.text, fontSize: 13, display: "flex", alignItems: "center", gap: 6, justifyContent: "space-between",
                }}>
                  <span style={{ display: "flex", alignItems: "center", gap: 6 }}><Users size={12} color={C.blueLight} /> {g.nombre}</span>
                  {sinLeer > 0 && <span style={{ background: C.gold, color: "#141A22", borderRadius: 999, minWidth: 16, height: 16, fontSize: 10, fontWeight: 700, display: "flex", alignItems: "center", justifyContent: "center", padding: "0 4px" }}>{sinLeer}</span>}
                </button>
              );
            })}
          </div>
        ...
        {contactosFiltrados.map(({ contacto: c, msgs }) => {
          const activoAqui = activo?.tipo === "directo" && activo.id === c.correoLogin;
          const sinLeer = activoAqui ? 0 : noLeidos(msgs, c.correoLogin);
          return (
          <div key={c.correoLogin} style={{ display: "flex", alignItems: "center", gap: 4 }}>
            <button onClick={() => setActivo({ tipo: "directo", id: c.correoLogin })} style={{
              flex: 1, textAlign: "left", background: activoAqui ? C.navy2 : "none", border: "none", borderRadius: 9,
              padding: "8px 10px", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "space-between", gap: 6,
            }}>
              <span>
...
function EntrarConCodigo({ cursos, onEncontrado, label }) {
  ...
  const buscar = () => {
    ...
    const resultado = onEncontrado(c);
    if (typeof resultado === "string" && resultado) { setError(resultado); return; }
    setCodigo("");
  };
  return (
      ...
      {tab === "cursos" && (
        cursoVisitado ? (
          ...
          <div>
            ...
            <CursosTab centro={centro} cursos={misCursos} maestros={staff.filter((s) => s.rol === "maestro")} setCursos={setCursos} setUsuarios={setUsuarios} onVerCurso={setCursoVisitadoId} />
          </div>
        )
      )}
...
function CursosTab({ centro, cursos, maestros, setCursos, setUsuarios, onVerCurso }) {
  ...
  const eliminar = (c) => {
    if (!confirm(`¿Eliminar el curso "${c.nombre}"? Esta acción no se puede deshacer. Los estudiantes seguirán en sus otros cursos, si tienen.`)) return;
    setCursos((cs) => cs.filter((x) => x.id !== c.id));
    setUsuarios((us) => us.map((u) => estaEnCurso(u, c.id) ? { ...u, cursosIds: cursosDe(u).filter((id) => id !== c.id) } : u));
  };
          ...
          {cursos.map((c) => (
            <div key={c.id} style={{ background: C.panel, border: `1px solid ${C.line}`, borderRadius: 10, padding: "11px 14px", display: "flex", justifyContent: "space-between", flexWrap: "wrap", gap: 8, alignItems: "center" }}>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                ...
                <button type="button" onClick={() => onVerCurso && onVerCurso(c.id)} style={{ ...btn(false, "ghost2"), width: "auto", fontSize: 11.5, padding: "6px 10px" }}>Ver curso</button>
...
function AvisosTab({ sesion, centro, staff, cursos, alertas, setAlertas, enviarCorreo }) {
  ...
  const estudiantesDeCurso = (id) => staff.filter((u) => u.rol === "estudiante" && estaEnCurso(u, Number(id)));
...
function StaffDashboard({ sesion, centro, usuarios, setUsuarios, cursos, setCursos, biblioteca, setBiblioteca, chats, setChats, grupos, setGrupos, alertas, setAlertas, enviarCorreo, onSalir }) {
  ...
  const [cursosInvitadosIds, setCursosInvitadosIds] = useState([]);
      ...
      <div style={{ width: "100%", maxWidth: 900, background: tema.fondoPagina, borderRadius: 24, padding: "22px 22px 30px" }}>
        ...
        {tab === "cursos" && sesion.rol === "maestro" && (
          <MisCursosMaestro
            ...
            cursosInvitadosIds={cursosInvitadosIds} setCursosInvitadosIds={setCursosInvitadosIds}
          />
        )}
...
function AvisosMaestro({ sesion, centro, cursos, usuarios, alertas, setAlertas, enviarCorreo }) {
  ...
  const estudiantes = usuarios.filter((u) => u.rol === "estudiante" && estaEnCurso(u, curso.id));
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 12, maxWidth: 480 }}>
      ...
      <select style={input} value={cursoId} onChange={(e) => setCursoId(e.target.value)}>
        {cursos.map((c) => <option key={c.id} value={c.id}>{c.nombre} ({usuarios.filter((u) => u.rol === "estudiante" && estaEnCurso(u, c.id)).length} estudiantes)</option>)}
      </select>
...
function MisCursosMaestro({ sesion, centro, cursos, todosCursos, setCursos, usuarios, setUsuarios, enviarCorreo, cursosInvitadosIds, setCursosInvitadosIds }) {
  const listaBusqueda = todosCursos || cursos;
  const cursosVisitados = listaBusqueda.filter((c) => cursosInvitadosIds.includes(c.id) && !cursos.some((mc) => mc.id === c.id));
  const listaCombinada = [...cursos, ...cursosVisitados];
  const [seleccionado, setSeleccionado] = useState(cursos[0]?.id || null);
  const curso = listaCombinada.find((c) => c.id === seleccionado) || listaCombinada[0] || null;
  ...
  const entrarConCodigo = (c) => {
    if (!cursosInvitadosIds.includes(c.id)) setCursosInvitadosIds((ids) => [...ids, c.id]);
    setSeleccionado(c.id);
  };

  const dejarDeVisitar = (id) => {
    setCursosInvitadosIds((ids) => ids.filter((x) => x !== id));
    if (seleccionado === id) setSeleccionado(cursos[0]?.id || null);
  };

  return (
    <div>
      <EntrarConCodigo cursos={listaBusqueda} onEncontrado={entrarConCodigo} label="Entrar a otro curso con código (aunque no seas su profesor asignado)" />
      <div style={{ display: "flex", gap: 20, flexWrap: "wrap" }}>
        <div style={{ display: "flex", flexDirection: "column", gap: 6, minWidth: 190 }}>
          {cursos.length === 0 && cursosVisitados.length === 0 && <div style={{ color: C.textDim, fontSize: 12.5, marginBottom: 6 }}>Todavía no tienes cursos. Crea el primero.</div>}
          {cursos.length > 0 && <div style={{ color: C.textDim, fontSize: 10.5, fontFamily: FM, marginTop: 2 }}>MIS CURSOS</div>}
          {cursos.map((c) => (
          ...
          {cursosVisitados.length > 0 && <div style={{ color: C.textDim, fontSize: 10.5, fontFamily: FM, marginTop: 8 }}>VISITANDO CON CÓDIGO</div>}
          {cursosVisitados.map((c) => (
            <div key={c.id} style={{ display: "flex", alignItems: "center", gap: 4 }}>
              <button onClick={() => setSeleccionado(c.id)} style={{ flex: 1, textAlign: "left", background: (curso && curso.id === c.id) ? C.navy2 : "none", border: "none", borderRadius: 9, padding: "9px 11px", cursor: "pointer", color: C.text, fontSize: 13.5, display: "flex", alignItems: "center", gap: 6 }}>
                {c.nombre} <Badge color={C.gold}>visita</Badge>
              </button>
              <button type="button" onClick={() => dejarDeVisitar(c.id)} title="Dejar de visitar este curso" style={{ background: "none", border: "none", color: C.textDim, cursor: "pointer", display: "flex", padding: 4 }}><X size={13} /></button>
            </div>
          ))}
          {creando ? (
...
// ============================================================
const JAAS_APP_ID = "vpaas-magic-cookie-0765e974aa644e15ba065b893753e8a9";
...
function SalonVideoJitsi({ salaId, sesion, nombreCentro, tema, onSalir }) {
  ...
  const esModerador = ["director", "maestro"].includes(sesion.rol);
  ...
  useEffect(() => {
    ...
    async function iniciar() {
      // 1) Pedimos un token firmado a nuestra función de Netlify (la clave privada
      //    nunca sale del servidor). Sin este token, JaaS no deja entrar a nadie.
      let token = null;
      try {
        const params = new URLSearchParams({
          name: sesion.nombre || "Invitado",
          email: sesion.correoReal || "",
          moderator: esModerador ? "1" : "0",
        });
        const res = await fetch(`/.netlify/functions/jitsi-token?${params.toString()}`);
      ...
      // 2) Cargamos el script de la videollamada, ya con el token listo.
      const arrancar = () => {
        if (cancelado || !window.JitsiMeetExternalAPI || !containerRef.current) return;
        containerRef.current.innerHTML = "";
        const api = new window.JitsiMeetExternalAPI(JAAS_DOMAIN, {
          roomName: `${JAAS_APP_ID}/${salaId}`,
          jwt: token,
          ...
          configOverwrite: {
            // Mostramos la pantalla previa de Jitsi (con botón "Unirse"): así el
            // navegador pide cámara/micrófono de forma normal y visible, en vez
            // de intentar entrar solo en silencio y quedarse colgado si algo falla.
        ...
        // En cuanto la sala termina de cargar (aunque todavía no se haya unido,
        // por ejemplo está viendo la pantalla previa), ya no está "colgado".
        api.addEventListener("videoConferenceJoined", () => setCargando(false));
        api.addEventListener("participantRoleChanged", () => setCargando(false));
      ...
      if (window.JitsiMeetExternalAPI) {
        ...
        if (existente) {
          existente.addEventListener("load", arrancar);
        } else {
    ...
    // Si en 20 segundos no cargó nada (ni la pantalla previa), avisamos en vez
    // de dejar el círculo de "Conectando…" dando vueltas para siempre.
    const limite = setTimeout(() => setTiempoAgotado(true), 20000);
  ...
  const reintentar = () => { setErrorCarga(false); setTiempoAgotado(false); setIntento((n) => n + 1); };
  ...
  if (errorCarga) {
    return (
      <div style={{ background: C.panel, border: `1px solid ${C.line}`, borderRadius: 12, padding: 20 }}>
        ...
        <div style={{ display: "flex", gap: 8, marginTop: 10 }}>
          <button type="button" onClick={reintentar} style={{ ...btn(false), width: "auto" }}>Reintentar</button>
          <button type="button" onClick={onSalir} style={{ ...btn(false, "ghost2"), width: "auto" }}>← Salir</button>
        </div>
      </div>
    );
  }
  ...
  return (
    <div style={{ background: C.panel, border: `1px solid ${C.line}`, borderRadius: 12, padding: 10 }}>
      ...
      {cargando && tiempoAgotado && (
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 10, background: C.navy2, border: `1px solid ${C.line}`, borderRadius: 10, padding: "10px 13px", marginBottom: 10, flexWrap: "wrap" }}>
          <span style={{ color: C.textDim, fontSize: 12.5 }}>Esto está tardando más de lo normal. Puede ser tu conexión o el servidor de video.</span>
          <button type="button" onClick={reintentar} style={{ ...btn(false, "ghost2"), width: "auto", fontSize: 12 }}>Reintentar</button>
        </div>
      )}
      <div ref={containerRef} style={{ borderRadius: 10, overflow: "hidden", height: 560, background: "#000" }} />
    </div>
  );
}
...
function CursoMaestroDetalle({ sesion, centro, curso, setCursos, usuarios, setUsuarios, enviarCorreo }) {
  ...
  return (
    <div style={{ flex: 1, minWidth: 280 }}>
      ...
      {subtab === "estudiantes" && <EstudiantesCurso sesion={sesion} centro={centro} curso={curso} estudiantes={estudiantes} usuarios={usuarios} setUsuarios={setUsuarios} enviarCorreo={enviarCorreo} />}
...
function EstudiantesCurso({ sesion, centro, curso, estudiantes, usuarios, setUsuarios, enviarCorreo }) {
  ...
  const agregar = async (e) => {
    ...
    const passwordTemporal = randPass();
    setUsuarios((us) => [...us, {
      correoLogin, cedula: cedula.trim(), password: passwordTemporal, rol: "estudiante", centroId: centro.id, cursosIds: [curso.id],
  ...
  // Buscar entre estudiantes YA registrados en el centro (por cédula o correo institucional/personal),
  // para agregarlos a este curso sin crear una cuenta duplicada.
  const q = busqueda.trim().toLowerCase();
  const resultadosBusqueda = q.length < 2 ? [] : usuarios.filter((u) =>
    u.rol === "estudiante" && u.centroId === centro.id && !estaEnCurso(u, curso.id) && (
      (u.cedula || "").toLowerCase().includes(q) ||
      (u.correoLogin || "").toLowerCase().includes(q) ||
      (u.correoReal || "").toLowerCase().includes(q) ||
      (u.nombre || "").toLowerCase().includes(q)
    )
  ).slice(0, 8);

  const agregarExistente = (u) => {
    if (cursosDe(u).length >= MAX_CURSOS_ESTUDIANTE) {
      setError(`${u.nombre} ya está inscrito en ${MAX_CURSOS_ESTUDIANTE} cursos, el máximo permitido. Debe salir de uno antes de entrar a este.`);
      return;
    }
    setError("");
    setUsuarios((us) => us.map((x) => x.correoLogin === u.correoLogin ? { ...x, cursosIds: [...cursosDe(x), curso.id] } : x));
    setBusqueda("");
  };

  const quitarDeEsteCurso = (u) => {
    if (!confirm(`¿Quitar a ${u.nombre} de "${curso.nombre}"? Su cuenta sigue activa y no pierde acceso a sus otros cursos.`)) return;
    setUsuarios((us) => us.map((x) => x.correoLogin === u.correoLogin ? { ...x, cursosIds: cursosDe(x).filter((id) => id !== curso.id) } : x));
  };
  ...
  return (
    <div>
      ...
      {mostrar && (
        <div style={{ background: C.panel, border: `1px solid ${C.line}`, borderRadius: 12, padding: 16, marginBottom: 16, display: "flex", flexDirection: "column", gap: 11 }}>
          <div style={{ display: "flex", gap: 4, background: C.navy2, border: `1px solid ${C.line}`, borderRadius: 9, padding: 3, width: "fit-content" }}>
            <button type="button" onClick={() => setModo("nuevo")} style={{ background: modo === "nuevo" ? useTema().grad : "none", color: modo === "nuevo" ? "#fff" : C.textDim, border: "none", borderRadius: 7, padding: "7px 12px", cursor: "pointer", fontSize: 12 }}>Estudiante nuevo</button>
            <button type="button" onClick={() => setModo("existente")} style={{ background: modo === "existente" ? useTema().grad : "none", color: modo === "existente" ? "#fff" : C.textDim, border: "none", borderRadius: 7, padding: "7px 12px", cursor: "pointer", fontSize: 12 }}>Ya registrado en el centro</button>
          </div>
          ...
          {modo === "nuevo" ? (
            ...
            <>
              <p style={{ color: C.textDim, fontSize: 12, margin: 0 }}>Busca por cédula, correo o nombre entre los estudiantes ya registrados en {centro.nombre}. Un estudiante puede estar en hasta {MAX_CURSOS_ESTUDIANTE} cursos a la vez.</p>
              <input style={input} placeholder="Cédula, correo o nombre" value={busqueda} onChange={(e) => setBusqueda(e.target.value)} />
              <ErrorBox>{error}</ErrorBox>
              {q.length >= 2 && (
                resultadosBusqueda.length === 0 ? (
                  <div style={{ color: C.textDim, fontSize: 12.5 }}>Sin coincidencias (o ya está en este curso).</div>
                ) : (
                  <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                    {resultadosBusqueda.map((u) => (
                      <div key={u.correoLogin} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", background: C.navy2, border: `1px solid ${C.line}`, borderRadius: 8, padding: "8px 11px", gap: 8, flexWrap: "wrap" }}>
                        <div>
                          <div style={{ color: C.text, fontSize: 13 }}>{u.nombre}</div>
                          <div style={{ color: C.textDim, fontSize: 11, fontFamily: FM }}>{u.correoLogin} · {cursosDe(u).length}/{MAX_CURSOS_ESTUDIANTE} cursos</div>
                        </div>
                        <button type="button" onClick={() => agregarExistente(u)} style={{ ...temaBtn(useTema(), false, "ghost2"), width: "auto", fontSize: 12 }}><Plus size={13} /> Agregar</button>
                      </div>
                    ))}
                  </div>
                )
      ...
      {estudiantes.length === 0 ? <div style={{ color: C.textDim, fontSize: 13 }}>Todavía no hay estudiantes en este curso. Comparte el código <b style={{ color: C.gold }}>{curso.codigoEstudiante}</b> y el código de la escuela para que se registren solos, o agrégalos aquí.</div> : (
        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          <div style={{ color: C.textDim, fontSize: 11, fontFamily: FM }}>Toca el nombre para ver la ficha completa (suspender/eliminar la cuenta). "Quitar de este curso" solo lo saca de esta aula, sin tocar su cuenta.</div>
          {estudiantes.map((u) => (
            <div key={u.correoLogin} style={{ background: C.panel, border: `1px solid ${u.suspendido ? C.red + "66" : C.line}`, borderRadius: 10, padding: "10px 13px", display: "flex", justifyContent: "space-between", flexWrap: "wrap", gap: 8, alignItems: "center" }}>
              <button type="button" onClick={() => setVerFicha(u)} style={{ background: "none", border: "none", cursor: "pointer", textAlign: "left", padding: 0 }}>
              ...
              <button type="button" onClick={() => quitarDeEsteCurso(u)} title="Quitar solo de este curso" style={{ background: "none", border: `1px solid ${C.line}`, borderRadius: 7, padding: "5px 9px", cursor: "pointer", color: C.textDim, fontSize: 11, display: "flex", alignItems: "center", gap: 5 }}>
                <X size={12} /> Quitar del curso
              </button>
            </div>
          ))}
        </div>
...
function EstudianteDashboard({ sesion, setSesion, centro, usuarios, setUsuarios, cursos, setCursos, biblioteca, setBiblioteca, chats, setChats, grupos, setGrupos, alertas, onSalir }) {
  ...
  const misCursos = cursos.filter((c) => estaEnCurso(sesion, c.id));
  const [cursoActivoId, setCursoActivoId] = useState(misCursos[0]?.id ?? null);
  const curso = misCursos.find((c) => c.id === cursoActivoId) || misCursos[0] || null;
  ...
  const misAlertas = (alertas || []).filter((a) => a.centroId === centro?.id && (a.cursoId ? estaEnCurso(sesion, a.cursoId) : true));
  const tema = temaDe(centro);

  const unirseConCodigo = (c) => {
    if (estaEnCurso(sesion, c.id)) return "Ya estás inscrito en ese curso.";
    if (cursosDe(sesion).length >= MAX_CURSOS_ESTUDIANTE) {
      return `Ya estás en el máximo de ${MAX_CURSOS_ESTUDIANTE} cursos a la vez. Sal de uno antes de entrar a otro.`;
    }
    setUsuarios((us) => us.map((u) => u.correoLogin === sesion.correoLogin ? { ...u, cursosIds: [...cursosDe(u), c.id] } : u));
    setSesion((s) => ({ ...s, cursosIds: [...cursosDe(s), c.id] }));
    setCursoActivoId(c.id);
  };

  const salirDelCurso = (c) => {
    if (!confirm(`¿Salir de "${c.nombre}"? Puedes volver a entrar después con el código, si sigue disponible.`)) return;
    setUsuarios((us) => us.map((u) => u.correoLogin === sesion.correoLogin ? { ...u, cursosIds: cursosDe(u).filter((id) => id !== c.id) } : u));
    setSesion((s) => ({ ...s, cursosIds: cursosDe(s).filter((id) => id !== c.id) }));
    if (cursoActivoId === c.id) setCursoActivoId(null);
  };
  ...
  return (
    <TemaContext.Provider value={tema}>
      <div style={{ width: "100%", maxWidth: 860, background: tema.fondoPagina, borderRadius: 24, padding: "22px 22px 30px" }}>
        ...
        {(tab === "aula" || tab === "calificaciones") && misCursos.length > 0 && (
          <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginBottom: 14 }}>
            {misCursos.map((c) => (
              <button key={c.id} onClick={() => setCursoActivoId(c.id)} style={{
                background: curso?.id === c.id ? tema.grad : C.panel, color: curso?.id === c.id ? "#fff" : C.textDim,
                border: `1px solid ${curso?.id === c.id ? "transparent" : C.line}`, borderRadius: 9, padding: "8px 13px", cursor: "pointer", fontSize: 12.5, display: "flex", alignItems: "center", gap: 6,
              }}>
                {c.nombre}
              </button>
            ))}
            {misCursos.length < MAX_CURSOS_ESTUDIANTE && (
              <span style={{ color: C.textDim, fontSize: 11, alignSelf: "center", fontFamily: FM }}>{misCursos.length}/{MAX_CURSOS_ESTUDIANTE} cursos</span>
            )}
          </div>
        )}
            ...
            <div>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 10, marginBottom: 10 }}>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                  ...
                  <button type="button" onClick={() => salirDelCurso(curso)} title="Salir de este curso" style={{ background: "none", border: `1px solid ${C.line}`, borderRadius: 7, padding: "6px 8px", cursor: "pointer", color: C.textDim, display: "flex" }}><X size={13} /></button>
                </div>
              </div>
              {subAula === "materiales" && (
              ...
              {misCursos.length < MAX_CURSOS_ESTUDIANTE && (
                <div style={{ marginTop: 20 }}>
                  <EntrarConCodigo cursos={cursos.filter((c) => c.centroId === centro.id)} onEncontrado={unirseConCodigo} label="ENTRAR A OTRO CURSO CON CÓDIGO" />
                </div>
              )}
            </div>
          )
