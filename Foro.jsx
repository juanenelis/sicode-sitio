...
export default function ForoPanel({ checkAdminCreds }) {
    ...
    <div style={{ background: C.panel, border: `1px solid ${C.line}`, borderRadius: 16, padding: 20, position: "sticky", top: 20 }}>
      ...
      {temaActivo ? (
        ...
        <>
          ...
          {creandoTema ? (
            <NuevoTema checkAdminCreds={checkAdminCreds} onCreado={() => { setCreandoTema(false); recargarTemas(); }} onCancelar={() => setCreandoTema(false)} />
          ) : (
            <button onClick={() => setCreandoTema(true)} style={{ background: "none", border: `1px dashed ${C.line}`, color: C.textDim, borderRadius: 9, padding: "9px 12px", cursor: "pointer", fontSize: 12, display: "flex", alignItems: "center", gap: 6, width: "100%", justifyContent: "center" }}>
              <Lock size={11} /> Soy SICODE — crear tema
            </button>
          )}
        </>
      )}
...
function NuevoTema({ checkAdminCreds, onCreado, onCancelar }) {
  ...
  const entrar = (e) => {
    e.preventDefault();
    if (checkAdminCreds(user, pwd)) { setAutenticado(true); setPwdError(false); }
  ...
  if (!autenticado) {
    return (
      <form onSubmit={entrar} style={{ display: "flex", flexDirection: "column", gap: 8, background: C.navy2, border: `1px solid ${C.line}`, borderRadius: 10, padding: 12 }}>
        ...
        {pwdError && <div style={{ color: C.red, fontSize: 11.5 }}>Incorrecto.</div>}
        <div style={{ display: "flex", gap: 6 }}>
          <button type="submit" style={{ background: C.gold, color: "#141A22", border: "none", borderRadius: 8, padding: "7px 12px", fontWeight: 700, cursor: "pointer", fontSize: 12 }}>Entrar</button>
