...
import {
  ...
  Users, BookOpen, LogOut, ShieldCheck, AlertCircle, Mail, RotateCcw, X,
...
import {
  collection, doc, setDoc, addDoc, updateDoc, deleteDoc, onSnapshot, arrayUnion,
...
export default function App() {
  // ---- "Base de datos" simulada (el sitio de UNA sola institución) ----
  const [centro, setCentro] = useState(null); // { id:"principal", nombre, mision, ... } o null si aún no se configura
  ...
  const enviarCorreo = (to, subject, body) => setCorreos((c) => [{ id: Date.now() + Math.random(), to, subject, body, fecha: new Date().toLocaleString() }, ...c]);

  const reiniciar = () => {
    setCentro(null); setUsuarios([]); setCursos([]); setBiblioteca([]); setChats({}); setGrupos([]); setAlertas([]); setCorreos([]);
    setSesion(null); setView("landing");
  };
  const cerrarSesion = () => { setSesion(null); setView("landing"); };

  return (
    <div style={{ minHeight: "100vh", background: C.navy, fontFamily: FB, display: "flex", flexDirection: "column", position: "relative", overflow: "hidden" }}>
      <GlobalStyle />
      <AnimatedBackdrop />

      <header style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "18px 5vw", flexWrap: "wrap", gap: 10, position: "relative", zIndex: 2 }}>
        <button onClick={() => !sesion && setView("landing")} style={{ display: "flex", alignItems: "center", gap: 10, background: "none", border: "none", cursor: sesion ? "default" : "pointer" }}>
          {centro?.logoUrl ? (
            <img src={centro.logoUrl} alt="" style={{ width: 36, height: 36, borderRadius: 9, objectFit: "cover" }} />
          ) : (
            <div style={{ width: 36, height: 36, borderRadius: 9, background: "linear-gradient(135deg,#3B7AF0,#9B7BF0)", display: "flex", alignItems: "center", justifyContent: "center", boxShadow: "0 0 20px rgba(91,154,245,0.5)" }}>
              <GraduationCap size={19} color="#fff" />
            </div>
          )}
          <div style={{ textAlign: "left" }}>
            <div style={{ color: C.text, fontFamily: FD, fontWeight: 700, fontSize: 17, lineHeight: 1 }}>{centro?.nombre || "SICODE Educa"}</div>
            <div style={{ color: C.textDim, fontSize: 10.5, fontFamily: FM }}>{centro ? "sitio del centro" : "vista previa — sitio sin configurar"}</div>
          </div>
        </button>
        <div style={{ display: "flex", gap: 8 }}>
          {!sesion && (
            <>
              <button onClick={() => setView("landing")} style={{ ...ghost, border: `1px solid ${C.line}`, borderRadius: 8, padding: "7px 12px" }}>Inicio</button>
              <button onClick={() => setView("login")} style={{ background: `linear-gradient(135deg,#3B7AF0,#9B7BF0)`, border: "none", color: "#fff", borderRadius: 8, padding: "7px 13px", display: "flex", alignItems: "center", gap: 6, cursor: "pointer", fontSize: 12.5, fontWeight: 600 }}>
                <Lock size={14} /> Iniciar sesión
              </button>
            </>
          )}
          <button onClick={() => setShowInbox(true)} style={{ ...ghost, border: `1px solid ${C.line}`, borderRadius: 8, padding: "7px 12px", display: "flex", alignItems: "center", gap: 6 }}>
            <Mail size={14} /> Correos ({correos.length})
          </button>
          <button onClick={reiniciar} style={{ ...ghost, border: `1px solid ${C.line}`, borderRadius: 8, padding: "7px 12px", display: "flex", alignItems: "center", gap: 6 }}>
            <RotateCcw size={14} /> Reiniciar demo
          </button>
        </div>
      </header>

      <main style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", padding: "1vh 5vw 6vh", position: "relative", zIndex: 2 }}>
        {!sesion && view === "landing" && (
          <SiteHome
            centro={centro}
            onEntrar={() => setView("login")}
            onInvitacion={() => setView("invitacion")}
            onConfigurar={() => setView("config")}
          />
        )}

        {!sesion && view === "config" && !centro?.configurada && (
          <ConfiguracionInicial
            setCentro={setCentro}
            setUsuarios={setUsuarios}
            onListo={(c, director) => { setCentro(c); setSesion(director); setView("landing"); }}
          />
        )}
        ...
        {!sesion && view === "login" && (
          centro?.configurada ? (
            ...
            <Card glow>
              <div style={{ display: "flex", alignItems: "center", gap: 9, marginBottom: 14 }}><AlertCircle size={19} color={C.gold} /><h1 style={{ color: C.text, fontFamily: FD, fontSize: 18, margin: 0 }}>Este sitio aún no está configurado</h1></div>
              <p style={{ color: C.textDim, fontSize: 13 }}>Antes de poder iniciar sesión, alguien debe configurar el centro (nombre, misión, redes sociales, etc.) y crear la cuenta de Director.</p>
              <button type="button" onClick={() => setView("config")} style={{ ...btn(false), width: "auto" }}><Sparkles size={15} /> Configurar el sitio ahora</button>
            </Card>
          )
        )}
        ...
        {!sesion && view === "invitacion" && (
          centro?.configurada ? (
            ...
            <Card glow>
              <p style={{ color: C.textDim, fontSize: 13 }}>Este sitio todavía no está configurado. Pide a tu profesor o director que lo configure primero.</p>
              <button type="button" onClick={() => setView("landing")} style={ghost}>← Volver</button>
            </Card>
          )
        )}
      ...
      {showInbox && <InboxModal correos={correos} onClose={() => setShowInbox(false)} />}
    </div>
  );
}
...
function InboxModal({ correos, onClose }) {
  return (
    <div onClick={onClose} style={{ position: "fixed", inset: 0, background: "rgba(2,6,16,0.75)", zIndex: 100, display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}>
      <div onClick={(e) => e.stopPropagation()} style={{ background: C.panel, border: `1px solid ${C.line}`, borderRadius: 16, padding: 24, maxWidth: 520, width: "100%", maxHeight: "75vh", overflowY: "auto", position: "relative" }}>
        <button onClick={onClose} style={{ position: "absolute", top: 14, right: 14, background: "none", border: "none", color: C.textDim, cursor: "pointer" }}><X size={18} /></button>
        <h3 style={{ color: C.text, fontFamily: FD, margin: "0 0 4px" }}>Bandeja de correos simulada</h3>
        <p style={{ color: C.textDim, fontSize: 12, marginTop: 0 }}>En el sitio real llegarían de verdad a cada correo.</p>
        {correos.length === 0 ? <div style={{ color: C.textDim, fontSize: 13 }}>Nada todavía.</div> : correos.map((c) => (
          <div key={c.id} style={{ borderTop: `1px solid ${C.line}`, padding: "11px 0" }}>
            <div style={{ color: C.blueLight, fontSize: 12, fontFamily: FM }}>Para: {c.to}</div>
            <div style={{ color: C.text, fontWeight: 600, fontSize: 13.5, margin: "3px 0" }}>{c.subject}</div>
            <div style={{ color: C.textDim, fontSize: 12.5, whiteSpace: "pre-wrap" }}>{c.body}</div>
            <div style={{ color: C.textDim, fontSize: 10.5, marginTop: 4 }}>{c.fecha}</div>
          </div>
        ))}
      </div>
    </div>
...
function ConfiguracionInicial({ setCentro, setUsuarios, onListo }) {
  const [tipoPersona, setTipoPersona] = useState("fisica");
  const [nombreCentro, setNombreCentro] = useState("");
  const [propNombre, setPropNombre] = useState(""); const [propApellido, setPropApellido] = useState("");
  const [cedula, setCedula] = useState(""); const [rnc, setRnc] = useState("");
  const [correoCentro, setCorreoCentro] = useState("");
  const [passwordDirector, setPasswordDirector] = useState("");
  const [preguntaSeguridad, setPreguntaSeguridad] = useState(PREGUNTAS_SEGURIDAD[0]);
  const [respuestaSeguridad, setRespuestaSeguridad] = useState("");
  const [mision, setMision] = useState(""); const [vision, setVision] = useState(""); const [objetivo, setObjetivo] = useState("");
  const [direccion, setDireccion] = useState(""); const [telefono, setTelefono] = useState(""); const [redesSociales, setRedesSociales] = useState({});
  const [colorPrimario, setColorPrimario] = useState(PALETAS[0].p); const [colorSecundario, setColorSecundario] = useState(PALETAS[0].s);
  const [logoPreview, setLogoPreview] = useState(null);
  const [error, setError] = useState("");

  const crear = () => {
    setError("");
    if (!nombreCentro.trim() || !correoCentro.trim() || (tipoPersona === "fisica" ? (!propNombre.trim() || !cedula.trim()) : (!rnc.trim()))) {
      setError("Completa el nombre del centro, tu identificación y el correo."); return;
    }
    if (!passwordDirector || passwordDirector.length < 4) { setError("Crea una contraseña de al menos 4 caracteres para tu acceso de Director."); return; }
    if (!respuestaSeguridad.trim()) { setError("Completa la respuesta de seguridad (sirve para recuperar el acceso)."); return; }
    const acr = acronimo(nombreCentro);
    const identificador = tipoPersona === "fisica" ? cedula.trim() : rnc.trim();
    const centro = {
      id: "principal", nombre: nombreCentro.trim(), acronimo: acr, codigoCentro: randCode(6), tipoPersona,
      propietarioNombre: tipoPersona === "fisica" ? `${propNombre} ${propApellido}` : nombreCentro.trim(),
      cedulaRnc: identificador, correoCentro: correoCentro.trim(), activo: true, suspendido: false,
      configurada: true, mision, vision, objetivo, logoUrl: logoPreview, direccion, telefono, redesSociales,
      colorPrimario, colorSecundario,
    };
    const director = {
      correoLogin: identificador, cedula: identificador, password: passwordDirector, debeCambiarPassword: false,
      rol: "director", centroId: centro.id, nombre: tipoPersona === "fisica" ? `${propNombre} ${propApellido}` : nombreCentro.trim(),
      correoReal: correoCentro.trim(), preguntaSeguridad, respuestaSeguridad: respuestaSeguridad.trim(),
    };
    setUsuarios((us) => [...us, director]);
    onListo(centro, director);
  };

  return (
    <Card maxWidth={620} glow>
      <div style={{ display: "flex", alignItems: "center", gap: 9, marginBottom: 6 }}><Sparkles size={20} color={C.gold} /><h1 style={{ color: C.text, fontFamily: FD, fontSize: 19, margin: 0 }}>Configura tu sitio</h1></div>
      <p style={{ color: C.textDim, fontSize: 12.5, marginTop: 0, marginBottom: 18 }}>Esto se hace una sola vez. Al terminar, entrarás directo como Director.</p>

      <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
        <label style={lbl}>NOMBRE DEL CENTRO EDUCATIVO</label>
        <input style={input} placeholder="Ej. Instituto San Rafael" value={nombreCentro} onChange={(e) => setNombreCentro(e.target.value)} />

        <div style={{ display: "flex", gap: 8 }}>
          <button type="button" onClick={() => setTipoPersona("fisica")} style={{ ...btn(false, tipoPersona === "fisica" ? undefined : "ghost2"), width: "auto", flex: 1 }}>Persona física</button>
          <button type="button" onClick={() => setTipoPersona("juridica")} style={{ ...btn(false, tipoPersona === "juridica" ? undefined : "ghost2"), width: "auto", flex: 1 }}>Persona jurídica</button>
        </div>
        {tipoPersona === "fisica" ? (
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
            <input style={input} placeholder="Tu nombre" value={propNombre} onChange={(e) => setPropNombre(e.target.value)} />
            <input style={input} placeholder="Tu apellido" value={propApellido} onChange={(e) => setPropApellido(e.target.value)} />
            <input style={{ ...input, gridColumn: "1 / -1" }} placeholder="Cédula (será tu usuario para entrar)" value={cedula} onChange={(e) => setCedula(e.target.value)} />
          </div>
        ) : (
          <input style={input} placeholder="RNC (será el usuario para entrar)" value={rnc} onChange={(e) => setRnc(e.target.value)} />
        )}
        <input style={input} placeholder="Correo del centro (sirve para recuperar el acceso)" value={correoCentro} onChange={(e) => setCorreoCentro(e.target.value)} />
        <input type="password" style={input} placeholder="Crea tu contraseña de Director" value={passwordDirector} onChange={(e) => setPasswordDirector(e.target.value)} />
        <div>
          <label style={lbl}>PREGUNTA DE SEGURIDAD (método alterno de recuperación de acceso)</label>
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            <select style={input} value={preguntaSeguridad} onChange={(e) => setPreguntaSeguridad(e.target.value)}>
              {PREGUNTAS_SEGURIDAD.map((p) => <option key={p} value={p}>{p}</option>)}
            </select>
        ...
        <div style={{ borderTop: `1px solid ${C.line}`, paddingTop: 14 }}>
          <label style={lbl}>INFORMACIÓN PÚBLICA DEL SITIO (la verán tus visitantes en el Inicio)</label>
          <label style={{ display: "flex", alignItems: "center", gap: 9, background: C.panel, border: `1px dashed ${C.line}`, borderRadius: 10, padding: 11, cursor: "pointer", width: "fit-content", marginBottom: 10 }}>
            <Upload size={15} color={C.blueLight} /><span style={{ color: C.textDim, fontSize: 12.5 }}>{logoPreview ? "Logo seleccionado ✓" : "Subir logo"}</span>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            <textarea style={{ ...input, minHeight: 50 }} placeholder="Misión" value={mision} onChange={(e) => setMision(e.target.value)} />
            <textarea style={{ ...input, minHeight: 50 }} placeholder="Visión" value={vision} onChange={(e) => setVision(e.target.value)} />
            <textarea style={{ ...input, minHeight: 50 }} placeholder="Objetivo" value={objetivo} onChange={(e) => setObjetivo(e.target.value)} />
          </div>
        </div>
        ...
        <div style={{ borderTop: `1px solid ${C.line}`, paddingTop: 12 }}>
          <label style={lbl}>DATOS DE CONTACTO (pie de página)</label>
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
        ...
        <div style={{ borderTop: `1px solid ${C.line}`, paddingTop: 12 }}>
          <label style={lbl}>REDES SOCIALES</label>
          <EditorRedesSociales redes={redesSociales} setRedes={setRedesSociales} />
        </div>

        <ErrorBox>{error}</ErrorBox>
        <button type="button" onClick={crear} style={{ ...btn(false), width: "auto" }}><CheckCircle2 size={15} /> Crear mi sitio y entrar</button>
      </div>
    </Card>
  );
}

// ============================= INICIO PÚBLICO DEL SITIO =============================
function SiteHome({ centro, onEntrar, onInvitacion, onConfigurar }) {
  if (!centro?.configurada) {
    return (
      <Card glow>
        <div style={{ textAlign: "center", padding: "10px 0" }}>
          <div style={{ width: 60, height: 60, margin: "0 auto 16px", borderRadius: 16, background: "linear-gradient(135deg,#3B7AF0,#9B7BF0)", display: "flex", alignItems: "center", justifyContent: "center", boxShadow: "0 0 30px rgba(91,154,245,0.45)" }}>
            <GraduationCap size={28} color="#fff" />
          </div>
          <h1 style={{ color: C.text, fontFamily: FD, fontSize: 21, margin: "0 0 8px" }}>Bienvenido a SICODE Educa</h1>
          <p style={{ color: C.textDim, fontSize: 13, maxWidth: 380, margin: "0 auto 20px" }}>Este sitio todavía no ha sido configurado. Si eres el director o administrador del centro, configúralo ahora.</p>
          <button type="button" onClick={onConfigurar} style={{ ...btn(false), width: "auto", margin: "0 auto" }}><Sparkles size={15} /> Configurar mi sitio</button>
        </div>
      </Card>
    );
  }
  ...
  const redes = Object.entries(centro.redesSociales || {}).filter(([, v]) => v && v.trim());
  const sinInfo = !centro.mision && !centro.vision && !centro.objetivo;

  return (
    <div style={{ width: "100%", maxWidth: 820 }}>
      <div style={{ borderRadius: 22, overflow: "hidden", border: `1px solid ${C.line}`, marginBottom: 22 }}>
        <div style={{ background: `linear-gradient(120deg, ${cp}, ${cs})`, padding: "40px 30px", textAlign: "center" }}>
          {centro.logoUrl ? (
            <img src={centro.logoUrl} alt="" style={{ width: 70, height: 70, borderRadius: 16, objectFit: "cover", margin: "0 auto 16px", display: "block", boxShadow: "0 10px 30px rgba(0,0,0,0.3)" }} />
          ) : (
            <div style={{ width: 70, height: 70, margin: "0 auto 16px", borderRadius: 16, background: "rgba(255,255,255,0.2)", display: "flex", alignItems: "center", justifyContent: "center" }}><GraduationCap size={32} color="#fff" /></div>
          )}
          <h1 style={{ color: "#fff", fontFamily: FD, fontSize: 26, margin: "0 0 8px" }}>{centro.nombre}</h1>
          <p style={{ color: "rgba(255,255,255,0.85)", fontSize: 13.5, margin: "0 0 20px" }}>Aula virtual, biblioteca y comunicación en un solo sitio</p>
          <div style={{ display: "flex", gap: 10, justifyContent: "center", flexWrap: "wrap" }}>
            <button type="button" onClick={onEntrar} style={{ background: "#fff", color: "#141A22", border: "none", borderRadius: 10, padding: "11px 20px", fontWeight: 700, cursor: "pointer", display: "flex", alignItems: "center", gap: 8, fontFamily: FB, fontSize: 14 }}><Lock size={15} /> Iniciar sesión</button>
            <button type="button" onClick={onInvitacion} style={{ background: "rgba(0,0,0,0.25)", color: "#fff", border: "none", borderRadius: 10, padding: "11px 20px", fontWeight: 700, cursor: "pointer", display: "flex", alignItems: "center", gap: 8, fontFamily: FB, fontSize: 14 }}><Link2 size={15} /> Soy estudiante nuevo</button>
          </div>
        </div>
      </div>
      ...
      {!sinInfo && (
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(220px,1fr))", gap: 14, marginBottom: 20 }}>
          {centro.mision && <InfoBlock titulo="Misión" texto={centro.mision} color={cp} />}
          {centro.vision && <InfoBlock titulo="Visión" texto={centro.vision} color={cs} />}
          {centro.objetivo && <InfoBlock titulo="Objetivo" texto={centro.objetivo} color={cp} />}
        </div>
      )}

      {redes.length > 0 && (
        <div style={{ background: C.panel, border: `1px solid ${C.line}`, borderRadius: 14, padding: 16, marginBottom: 20 }}>
          <div style={{ color: C.textDim, fontSize: 11, fontFamily: FM, marginBottom: 10 }}>SÍGUENOS</div>
          <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
            {redes.map(([k, v]) => {
              const info = REDES_DISPONIBLES.find((r) => r.k === k);
              const Icon = info?.icon || Globe;
              return (
                <a key={k} href={v} target="_blank" rel="noreferrer" style={{ display: "flex", alignItems: "center", gap: 7, color: cp, textDecoration: "none", background: C.navy2, border: `1px solid ${C.line}`, borderRadius: 9, padding: "8px 12px", fontSize: 12.5 }}>
                  <Icon size={14} /> {info?.l || k}
                </a>
              );
            })}
          </div>
      ...
      <SitioFooter centro={centro} />
    </div>
  );
}
// ============================= PRIMER INGRESO DEL DIRECTOR =============================
...
function Chat({ sesion, centro, usuarios, chats, setChats, grupos, setGrupos }) {
  ...
  return (
    <div style={{ display: "flex", gap: 14, height: 420, flexWrap: "wrap" }}>
      <div style={{ width: 210, borderRight: `1px solid ${C.line}`, overflowY: "auto", display: "flex", flexDirection: "column", gap: 4, paddingRight: 8 }}>
        ...
        {contactosFiltrados.map((c) => (
          <button key={c.correoLogin} onClick={() => setActivo({ tipo: "directo", id: c.correoLogin })} style={{
            textAlign: "left", background: activo?.tipo === "directo" && activo.id === c.correoLogin ? C.navy2 : "none", border: "none", borderRadius: 9,
          ...
          <div style={{ background: C.navy2, border: `1px solid ${C.line}`, borderRadius: 10, padding: 10, marginBottom: 10, display: "flex", flexDirection: "column", gap: 8, maxHeight: 200, overflowY: "auto" }}>
            ...
            {activoGrupo.miembros.map((correo) => (
              <div key={correo} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", fontSize: 12.5, color: C.text }}>
                <span>{nombreDe(correo)}{correo === activoGrupo.creadoPor && <span style={{ color: C.textDim, fontSize: 10.5 }}> (creador)</span>}</span>
...
function Biblioteca({ sesion, centro, biblioteca, setBiblioteca, readOnly }) {
  ...
  const publicar = (e) => {
    ...
    setBiblioteca((b) => [...b, {
      ...
      nombre: archivo.name, url: URL.createObjectURL(archivo), publicadoPor: sesion.nombre, publicadoPorRol: sesion.rol,
    ...
    <div>
      {!readOnly && (
        <div style={{ background: C.navy2, border: `1px solid ${C.line}`, borderRadius: 12, padding: 14, marginBottom: 16, display: "flex", flexDirection: "column", gap: 9 }}>
          ...
          <ErrorBox>{error}</ErrorBox>
          <button type="button" onClick={publicar} style={{ ...btn(false), width: "auto" }}><Plus size={15} /> {esEstudiante ? "Publicar material de apoyo" : "Publicar en biblioteca"}</button>
        </div>
      )}
...
function PersonalTab({ sesion, centro, staff, setUsuarios, enviarCorreo }) {
  ...
  const agregar = (e) => {
    ...
    enviarCorreo(correo, `Tienes acceso como ${ROLES[rol]} en ${centro.nombre}`, `Hola ${nombre}, entra con tu usuario (${correoLogin}) y esta contraseña temporal: ${passwordTemporal}. Te pedirá crear tu propia contraseña al entrar.`);
    setUltimo({ nombre, correoLogin, passwordTemporal });
  ...
  const enviarInvitacion = (e) => {
    ...
    enviarCorreo(correoInv, `Te invitaron a ${centro.nombre}`, `Te invitaron a unirte a ${centro.nombre} como ${ROLES[rolInv]}. Entra con el usuario ${correoLogin} y la contraseña temporal ${passwordTemporal}. Al entrar te pediremos completar tus datos.`);
    setUltimoInv({ correoLogin, passwordTemporal });
    setCorreoInv("");
  };
    ...
    <div>
      ...
      {modo === "completa" && (
        <div style={{ background: C.panel, border: `1px solid ${C.line}`, borderRadius: 14, padding: 18, marginBottom: 18, display: "flex", flexDirection: "column", gap: 11 }}>
          ...
          {ultimo && <OkBox>Listo. {ultimo.nombre} "recibió" correo con usuario <b style={{ fontFamily: FM }}>{ultimo.correoLogin}</b> y contraseña temporal <b style={{ fontFamily: FM }}>{ultimo.passwordTemporal}</b></OkBox>}
        </div>
      )}
      ...
      {modo === "invitacion" && (
        <div style={{ background: C.panel, border: `1px solid ${C.line}`, borderRadius: 14, padding: 18, marginBottom: 18, display: "flex", flexDirection: "column", gap: 11 }}>
          ...
          {ultimoInv && <OkBox>Invitación "enviada". Usuario <b style={{ fontFamily: FM }}>{ultimoInv.correoLogin}</b> y contraseña temporal <b style={{ fontFamily: FM }}>{ultimoInv.passwordTemporal}</b></OkBox>}
        </div>
      )}
      ...
      {staff.length === 0 ? <div style={{ color: C.textDim, fontSize: 13.5 }}>Sin personal todavía.</div> : (
        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          {staff.map((u, i) => (
            <div key={i} style={{ background: C.panel, border: `1px solid ${C.line}`, borderRadius: 10, padding: "10px 14px", display: "flex", justifyContent: "space-between", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
...
function SalonVideo({ salaId, sesion, onSalir }) {
  const miIdRef = useRef(slug(sesion.correoLogin || sesion.nombre) + "-" + Math.random().toString(36).slice(2, 8));
  const localVideoRef = useRef(null);
  const localStreamRef = useRef(null);
  const peersRef = useRef({});
  const conexionesCreadasRef = useRef(new Set());
  const candidatosAplicadosRef = useRef({});
  ...
  useEffect(() => {
    ...
    async function iniciar() {
      try {
        ...
        setError("No pudimos acceder a tu cámara/micrófono. Revisa los permisos del navegador.");
        return;
      }

      const participantesCol = collection(db, "salas", salaId, "participantes");
      const miDocRef = doc(participantesCol, miId);
      await setDoc(miDocRef, { nombre: sesion.nombre, rol: sesion.rol, entrada: Date.now() });

      const crearPeer = (otroId) => {
        if (peersRef.current[otroId]) return peersRef.current[otroId];
        const pc = new RTCPeerConnection(ICE_SERVERS);
        peersRef.current[otroId] = pc;
        (localStreamRef.current?.getTracks() || []).forEach((track) => pc.addTrack(track, localStreamRef.current));
        pc.ontrack = (ev) => setRemotos((r) => ({ ...r, [otroId]: ev.streams[0] }));
        pc.onconnectionstatechange = () => {
          if (["failed", "closed", "disconnected"].includes(pc.connectionState)) {
            setRemotos((r) => { const n = { ...r }; delete n[otroId]; return n; });
          }
        };
        return pc;
      };

      const manejarPar = async (otroId) => {
        if (otroId === miId || peersRef.current[otroId]) return;
        const [a, b] = [miId, otroId].sort();
        const soyOfferer = a === miId;
        const connectionId = `${a}__${b}`;
        const conexDocRef = doc(db, "salas", salaId, "conexiones", connectionId);
        conexionesCreadasRef.current.add(connectionId);
        const pc = crearPeer(otroId);

        if (soyOfferer) {
          pc.onicecandidate = (ev) => { if (ev.candidate) updateDoc(conexDocRef, { candOfferer: arrayUnion(JSON.stringify(ev.candidate)) }).catch(() => {}); };
          const offer = await pc.createOffer();
          await pc.setLocalDescription(offer);
          await setDoc(conexDocRef, { offer: JSON.stringify(offer), candOfferer: [], candAnswerer: [] }, { merge: true });
          const unsub = onSnapshot(conexDocRef, async (snap) => {
            const data = snap.data();
            if (!data) return;
            if (data.answer && !pc.currentRemoteDescription) {
              await pc.setRemoteDescription(JSON.parse(data.answer));
            }
            const cands = data.candAnswerer || [];
            const yaAplicados = candidatosAplicadosRef.current[connectionId] || 0;
            for (let i = yaAplicados; i < cands.length; i++) {
              try { await pc.addIceCandidate(JSON.parse(cands[i])); } catch (e) {}
            }
            candidatosAplicadosRef.current[connectionId] = cands.length;
          });
          unsubsRef.current.push(unsub);
        } else {
          pc.onicecandidate = (ev) => { if (ev.candidate) updateDoc(conexDocRef, { candAnswerer: arrayUnion(JSON.stringify(ev.candidate)) }).catch(() => {}); };
          const unsub = onSnapshot(conexDocRef, async (snap) => {
            const data = snap.data();
            ...
            if (!pc.currentRemoteDescription) {
              ...
              await updateDoc(conexDocRef, { answer: JSON.stringify(answer) });
            }
            const cands = data.candOfferer || [];
            const yaAplicados = candidatosAplicadosRef.current[connectionId] || 0;
            for (let i = yaAplicados; i < cands.length; i++) {
              try { await pc.addIceCandidate(JSON.parse(cands[i])); } catch (e) {}
            }
      ...
      const unsubParticipantes = onSnapshot(participantesCol, (snap) => {
        const lista = snap.docs.map((d) => ({ id: d.id, ...d.data() }));
        setParticipantes(lista);
        lista.forEach((p) => { if (p.id !== miId) manejarPar(p.id); });
      });
      unsubsRef.current.push(unsubParticipantes);
    }
    ...
    return () => {
      ...
      deleteDoc(doc(db, "salas", salaId, "participantes", miId)).catch(() => {});
      conexionesCreadasRef.current.forEach((cid) => deleteDoc(doc(db, "salas", salaId, "conexiones", cid)).catch(() => {}));
    };
  }, [salaId]);
  ...
  const otros = participantes.filter((p) => p.id !== miIdRef.current);

  return (
    <div style={{ background: C.panel, border: `1px solid ${C.line}`, borderRadius: 12, padding: 14 }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 10, flexWrap: "wrap", gap: 8 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 8, color: C.green, fontSize: 12.5 }}>
          <span style={{ width: 8, height: 8, borderRadius: "50%", background: C.green, display: "inline-block", boxShadow: `0 0 8px ${C.green}` }} /> En vivo · {otros.length + 1} persona{otros.length === 0 ? "" : "s"}
        </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(180px,1fr))", gap: 10 }}>
        <div style={{ position: "relative", borderRadius: 10, overflow: "hidden", background: "#000", aspectRatio: "4/3" }}>
          ...
          <div style={{ position: "absolute", bottom: 6, left: 8, color: "#fff", fontSize: 11, background: "rgba(0,0,0,0.5)", padding: "2px 7px", borderRadius: 6 }}>{sesion.nombre} (tú)</div>
        </div>
        {otros.map((p) => (
          <VideoRemoto key={p.id} stream={remotos[p.id]} nombre={p.nombre} />
        ))}
      </div>

      {listo && (
        <div style={{ display: "flex", gap: 8, marginTop: 14, justifyContent: "center" }}>
          <button type="button" onClick={toggleMic} style={{ ...btn(false, micOn ? "ghost2" : "danger"), width: "auto" }}>{micOn ? "Silenciar" : "Activar mic"}</button>
          <button type="button" onClick={toggleCam} style={{ ...btn(false, camOn ? "ghost2" : "danger"), width: "auto" }}>{camOn ? "Apagar cámara" : "Prender cámara"}</button>
        </div>
      )}
      <p style={{ color: C.textDim, fontSize: 10.5, marginTop: 10, marginBottom: 0 }}>Videollamada propia del sitio (WebRTC). Funciona mejor con grupos pequeños; en redes muy restringidas algún participante podría no conectar.</p>
    </div>
  );
}

function VideoRemoto({ stream, nombre }) {
  const ref = useRef(null);
  useEffect(() => { if (ref.current) ref.current.srcObject = stream || null; }, [stream]);
  return (
    <div style={{ position: "relative", borderRadius: 10, overflow: "hidden", background: "#000", aspectRatio: "4/3", display: "flex", alignItems: "center", justifyContent: "center" }}>
      {stream ? <video ref={ref} autoPlay playsInline style={{ width: "100%", height: "100%", objectFit: "cover" }} /> : <div style={{ color: C.textDim, fontSize: 11.5 }}>Conectando…</div>}
      <div style={{ position: "absolute", bottom: 6, left: 8, color: "#fff", fontSize: 11, background: "rgba(0,0,0,0.5)", padding: "2px 7px", borderRadius: 6 }}>{nombre}</div>
    </div>
  );
}

function VideollamadaCurso({ curso, sesion, actualizarCurso, rol }) {
  ...
  if (salaActiva) {
    ...
    return <SalonVideo salaId={salaId} sesion={sesion} onSalir={() => setSalaActiva(null)} />;
  }
  ...
  return (
    <div style={{ flex: 1, minWidth: 280 }}>
      ...
      <div style={{ display: "flex", gap: 4, marginBottom: 16, borderBottom: `1px solid ${C.line}` }}>
        {[{ k: "materiales", l: "Aula virtual", i: BookOpen }, { k: "estudiantes", l: "Estudiantes", i: Users }, { k: "videollamada", l: "Videollamada", i: Video }].map((t) => (
      ...
      {subtab === "videollamada" && (curso.suspendido
        ...
        : <VideollamadaCurso curso={curso} sesion={sesion} actualizarCurso={actualizarCurso} rol="maestro" />)}
    </div>
  );
}

function EstudiantesCurso({ centro, curso, estudiantes, setUsuarios, enviarCorreo }) {
  ...
  const agregar = (e) => {
    ...
    if (correo.trim()) enviarCorreo(correo.trim(), `Fuiste agregado a ${curso.nombre}`, `Hola ${nombre}, tu profesor te agregó al curso "${curso.nombre}". Entra con el usuario ${correoLogin} y la contraseña temporal ${passwordTemporal}.`);
    setUltimo({ nombre, correoLogin, passwordTemporal });
  ...
  return (
    <div>
      <button type="button" onClick={() => setMostrar((s) => !s)} style={{ ...btn(false, mostrar ? undefined : "ghost2"), width: "auto", marginBottom: 14 }}><UserPlus size={15} /> {mostrar ? "Cancelar" : "Agregar estudiante"}</button>
      {mostrar && (
        <div style={{ background: C.panel, border: `1px solid ${C.line}`, borderRadius: 12, padding: 16, marginBottom: 16, display: "flex", flexDirection: "column", gap: 9 }}>
          ...
          {ultimo && <OkBox>Listo. Usuario <b style={{ fontFamily: FM }}>{ultimo.correoLogin}</b> · contraseña temporal <b style={{ fontFamily: FM }}>{ultimo.passwordTemporal}</b></OkBox>}
        </div>
      )}
      {estudiantes.length === 0 ? <div style={{ color: C.textDim, fontSize: 13 }}>Todavía no hay estudiantes en este curso. Comparte el código <b style={{ color: C.gold }}>{curso.codigoEstudiante}</b> y el código de la escuela para que se registren solos, o agrégalos aquí.</div> : (
        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          {estudiantes.map((u) => (
            <div key={u.correoLogin} style={{ background: C.panel, border: `1px solid ${C.line}`, borderRadius: 10, padding: "10px 13px", display: "flex", justifyContent: "space-between", flexWrap: "wrap", gap: 6 }}>
              <span style={{ color: C.text, fontSize: 13.5 }}>{u.nombre}</span>
...
function PublicarMaterialForm({ sesion, onPublicar }) {
  ...
  const publicar = (e) => {
    ...
    if (tipo === "material") {
      ...
      onPublicar({ ...base, nombre: archivo.name, url: URL.createObjectURL(archivo) });
    } else if (tipo === "tarea") {
      onPublicar({ ...base, nombre: archivo?.name || null, url: archivo ? URL.createObjectURL(archivo) : null, formatosAceptados: formatos, entregas: [] });
    } else if (tipo === "foro") {
      const publicaciones = [];
      if (resumen.trim() || foroImagen || foroLink.trim()) {
        publicaciones.push({
          ...
          imagenUrl: foroImagen ? URL.createObjectURL(foroImagen) : null,
      ...
      if (limpias.length === 0) { setError("Agrega al menos una pregunta con dos opciones y marca la correcta."); return; }
  ...
  return (
    <div style={{ background: C.navy2, border: `1px solid ${C.line}`, borderRadius: 12, padding: 16, marginBottom: 16, display: "flex", flexDirection: "column", gap: 11 }}>
      ...
      <ErrorBox>{error}</ErrorBox>
      <button type="button" onClick={publicar} style={{ ...btn(false), width: "auto" }}><Plus size={15} /> Publicar</button>
    </div>
  );
}
...
function ForoPanel({ m, sesion, actualizarItem, rol }) {
  ...
  const responder = (targetId, { texto, imagen, link, linkTexto }) => {
    const nueva = {
      id: nuevoId(), autor: sesion.correoLogin, autorNombre: sesion.nombre, rol: sesion.rol, texto,
      imagenUrl: imagen ? URL.createObjectURL(imagen) : null, linkUrl: link || null, linkTexto: linkTexto || null,
  ...
  const publicarModelo = ({ texto, imagen, link, linkTexto }) => {
    const nueva = {
      id: nuevoId(), autor: sesion.correoLogin, autorNombre: sesion.nombre, rol: sesion.rol, texto,
      imagenUrl: imagen ? URL.createObjectURL(imagen) : null, linkUrl: link || null, linkTexto: linkTexto || null,
      fecha: new Date().toLocaleString(), respuestas: [],
...
function CarpetaPanel({ m, actualizarItem, rol }) {
  ...
  const agregar = () => {
    if (!archivo) return;
    actualizarItem((it) => ({ ...it, documentos: [...(it.documentos || []), { id: nuevoId(), nombre: archivo.name, url: URL.createObjectURL(archivo), fecha: new Date().toLocaleDateString() }] }));
    setArchivo(null);
  };
    ...
    <div style={{ marginTop: 8 }}>
      ...
      {rol === "maestro" && (
        <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
          ...
          <button type="button" onClick={agregar} style={{ ...btn(false), width: "auto", fontSize: 12, padding: "7px 12px" }}><Plus size={13} /> Agregar</button>
        </div>
      )}
    </div>
  );
...
function TareaEstudiante({ m, sesion, actualizarItem }) {
  const [archivo, setArchivo] = useState(null); const [error, setError] = useState("");
  const cerrado = estaVencido(m);
  ...
  const entregar = () => {
    ...
    const entrega = { estudianteCorreo: sesion.correoLogin, estudianteNombre: sesion.nombre, archivoNombre: archivo.name, archivoUrl: URL.createObjectURL(archivo), fecha: new Date().toLocaleString(), calificacion: null };
    ...
    <div style={{ marginTop: 8 }}>
      ...
      {cerrado ? <Badge color={C.red}>Esta tarea ya cerró, no se puede entregar.</Badge> : (
        <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
          ...
          <button type="button" onClick={entregar} style={{ ...btn(false), width: "auto", fontSize: 12, padding: "7px 12px" }}><CheckCircle2 size={13} /> {miEntrega ? "Volver a entregar" : "Entregar"}</button>
        </div>
      )}
      <ErrorBox>{error}</ErrorBox>
    </div>
...
function EstudianteDashboard({ sesion, setSesion, centro, usuarios, setUsuarios, cursos, setCursos, biblioteca, setBiblioteca, chats, setChats, grupos, setGrupos, alertas, onSalir }) {
  ...
  const cambiarFoto = (e) => { const f = e.target.files[0]; if (f) setFoto(URL.createObjectURL(f)); };
  ...
  return (
    <TemaContext.Provider value={tema}>
      <div style={{ width: "100%", maxWidth: 860, background: tema.fondoPagina, borderRadius: 24, padding: "22px 22px 30px" }}>
        <SitioHeader centro={centro} sesion={sesion} foto={foto} onFoto={cambiarFoto} onSalir={onSalir} tabs={tabs} tab={tab} setTab={setTab} subtitulo="Estudiante" />
          ...
          curso ? (
            curso.suspendido ? (
              ...
              <div>
                ...
                {subAula === "videollamada" && <VideollamadaCurso curso={curso} sesion={sesion} actualizarCurso={actualizarCurso} rol="estudiante" />}
              </div>
            )
          ) : (
            <div>
